                         ...
                         float* icing, ValuesDefined& fDefined, float undef)
{
  ...
  MIUTIL_OPENMP_PARALLEL(fsize, for reduction(+:n_undefined))
  for (int i = 0; i < fsize; i++) {
    if (calculations::is_defined(inAllDefined, airtemp[i], seatemp[i], u[i], v[i], sal[i], aice[i], undef) && aice[i] < 0.4) {
                         ...
                         const float alpha, const float zmin, const float zmax, ValuesDefined& fDefined, float undef)
{
  ...
  for (int i = 0; i < fsize; i++) {
    ...
    if (calculations::is_defined(inAllDefined, sal[i], wave[i], x_wind[i], y_wind[i], airtemp[i], rh[i], sst[i], p[i], aice[i], depth[i], undef) &&
        aice[i] < 0.4) {
  ...
  for (int i = 0; i < fsize; i++) {
    if (calculations::is_defined(inAllDefined, sal[i], wave[i], x_wind[i], y_wind[i], airtemp[i], rh[i], sst[i], p[i], aice[i], depth[i], undef) &&
      ...
      if (v < 1 || wave[i] < 0.1) {
        ...
        double Wrx = fabs(ux_);
        double Wry = fabs(uy_);
        double Wr = miutil::absval(ux_, uy_);
        // Adding weights:
