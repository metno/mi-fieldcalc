/*
  mi-fieldcalc - A Free Meteorological Visualisation Tool
...
*/
#ifndef MI_FIELDCALC_METCONSTANTS_H
#define MI_FIELDCALC_METCONSTANTS_H
...
const int N_EWT = 41;
const float ewt[N_EWT] = {.000034, .000089, .000220, .000517, .001155, .002472, .005080, .01005, .01921, .03553, .06356, .1111,  .1891,  .3139,
                          .5088,   .8070,   1.2540,  1.9118,  2.8627,  4.2148,  6.1078,  8.7192, 12.272, 17.044, 23.373, 31.671, 42.430, 56.236,
                          73.777,  95.855,  123.40,  157.46,  199.26,  250.16,  311.69,  385.56, 473.67, 578.09, 701.13, 845.28, 1013.25};
...
public:
  ...
  bool defined() const { return l >= 0 and l < N_EWT - 1; }
  ...
  float value() const { return ewt[l] + (ewt[l + 1] - ewt[l]) * (x - l); }
...
const float pLevelTable[nLevelTable] = {1000, 925, 850, 800, 700, 500, 400, 300, 250, 200, 150, 100, 70, 50, 30, 10};
const float fLevelTable[nLevelTable] = {5, 25, 50, 65, 100, 185, 235, 300, 340, 385, 445, 530, 605, 675, 780, 1020};
// obsolete table, kept in order to support old input files
const float fLevelTable_old[nLevelTable] = {0, 25, 50, 70, 100, 180, 240, 300, 340, 390, 450, 530, 600, 700, 800, 999};
...
const std::string VerticalName[7] = {"none", "pressure", "hybrid", "atmospheric", "isentropic", "oceandepth", "other"};

} // namespace Constants

inline float ms2knots(float ff)
{
  return ff * miutil::constants::ms2knots;
}

inline float knots2ms(float ff)
{
  return ff * miutil::constants::knots2ms;
}

} // namespace MetNo

#endif // MI_FIELDCALC_METCONSTANTS_H
