...
static const double ICAO_base_T[ICAO_n] = {288.15, 216.65, 216.65, 228.65, 270.65, 270.65, 214.65, 186.946};
static const double ICAO_base_p[ICAO_n] = {
    1013.15, 226.29806486313493, 54.743370958898005, 8.679301101236328, 1.1089482781849516, 0.6693192180209551, 0.0395600169484907, 0.0037334345211142398};
...
double ICAO_geo_altitude_from_pressure(double pressure)
{
  ...
  const int l = i - 1;
  const double lambda_l = ICAO_lambdas[l] / 1000, h_l = ICAO_base_h[l] * 1000, t_l = ICAO_base_T[l], p_l = ICAO_base_p[l];
...
double ICAO_pressure_from_geo_altitude(double altitude)
{
  ...
  const int l = i - 1;
  const double lambda_l = ICAO_lambdas[l] / 1000, altitude_l = ICAO_base_h[l] * 1000, t_l = ICAO_base_T[l], p_l = ICAO_base_p[l];
